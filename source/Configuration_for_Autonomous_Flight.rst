...
* **Configure the mass of the UAV**
	Open ```~/mrs_workspace/src/uav_core/ros_packages/mrs_uav_managers/config/simulation/t650/mass.yaml``` and adjust the mass: ```uav_mass: 4.95 [kg]```. This ensures that the controllers and trackers that use mass (e.g., for feedforward actions) use the hardware mass.
	
	Open ```~/mrs_workspace/src/simulation/ros_packages/mrs_simulation/models/mrs_robots_description/sdf/t650.sdf.jinja``` and adjust the mass: ```{%- set mass = 4.95 -%} {# [kg] #}```. This ensures that Gazebo simulates a UAV model with the hardware mass. 
