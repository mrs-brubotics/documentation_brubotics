...
First, we define the header. Then we have to orient the frame. To do so, we made a new function named ``FrameOrientation`` which is inspired by
...
Thus, we use another function named ``GiveTranslatedPoint``.
